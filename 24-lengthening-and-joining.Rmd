---
title: "24 - Lengthening and Joining"
...
# Structure
This document corresponds with step 5 of the LAPOP-dashboard repository. We will now lengthen the Bolivia merged, 2016, and 2018 files and combine them with their respective factored and unfactored versions.
...
### For 2016 Bolivia unfactored file
```{r}
...
### For 2018 Bolivia unfactored file
```{r}
...
```{r}
unfactor <- lengthen(bolivia_unique_unfactor)
unfactor16 <- lengthen_16(bolivia_16_unfactor)
unfactor18 <- lengthen_18(bolivia_18_unfactor)
```
...
### Applying for factored Bolivia datasets
```{r}
factor <- lengthen_factor(bolivia_unique_factor)
factor16 <- lengthen_16factor(bolivia_16_factor)
factor18 <- lengthen_18factor(bolivia_18_factor)
```
...
```{r}
#Bolivia
bolivia_clean <- unmerged(bolivia_unique_unfactor, bolivia_16_unfactor, bolivia_18_unfactor, bolivia_unique_factor, bolivia_16_factor, bolivia_18_factor)
```